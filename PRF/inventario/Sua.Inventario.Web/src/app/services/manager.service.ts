...
export class HttpManagerService {
  ...
  constructor(private http: HttpClient, private modalService: NgbModal) {
    ...
    this.urlExtAcc = `${environment.server}SuaApi.Accesos/api/${environment.version}`;
  }
  ...
  private handleError<T>(result?: T) {
    return (error: any): Observable<T> => {
      ...
      if (error.status === 400) {
        ...
        const urlPath = error.url.split('/');
        if (urlPath[6] === 'Inventario') {
          this.ShowNotification('Inventario Activo', 'Existe un inventario activo, debe cerrarlo antes de poder cargar uno nuevo',
          error.message, error.status, null);
        }
        else {
          this.ShowNotification('Dato existente', 'El dato ya se encuentra activo',
                    error.message, error.status, null);
