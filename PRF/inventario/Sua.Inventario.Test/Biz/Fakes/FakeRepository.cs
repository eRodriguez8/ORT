...
namespace Sua.Inventario.Test.Biz.Fakes
{
    public class FakeRepository<TEntity, TDataBase> : IGenericRepository<TEntity, TDataBase> where TEntity : class where TDataBase : DbContext
    {
        ...
        public IQueryable<TEntity> Get(Expression<Func<TEntity, bool>> filter = null, bool trackeable = false, Func<IQueryable<TEntity>, IOrderedQueryable<TEntity>> orderBy = null, string includeProperties = "")
        {
            AccessCounterGet++;
        ...
        public void BulkInsert(List<TEntity> entity)
        {
            AccessCounterInsert++;
            Data = entity;
        }
